...
class GovernDocs_CPT_Policy extends GovernDocs_CPT_Base {

    public function hooks() : void {

        if ( \GovernDocs\GovernDocs_Admin_Menu::is_post_type_disabled( 'policies' ) ) {
            return;
        }
    ...
    private function policy_shortcode( int $post_id ) : string {
        return '[governdocs_policy id="' . absint( $post_id ) . '"]';
    }

    public function updated_messages( array $messages ) : array {
        $post = get_post();

        if ( ! $post || 'governdocs_policy' !== $post->post_type ) {
            return $messages;
        }

        $shortcode = $this->policy_shortcode( (int) $post->ID );

        $shortcode_hint = sprintf(
            ' <span class="governdocs-shortcode-hint">%s</span>',
            sprintf(
                /* translators: %s: shortcode */
                esc_html__( 'Display this policy using the shortcode: %s', 'governdocs-document-governance' ),
                $shortcode
            )
        );
        ...
        $messages['governdocs_policy'] = array(
            0  => '',
            1  => esc_html__( 'Policy updated.', 'governdocs-document-governance' ) . $shortcode_hint,
            ...
            4  => esc_html__( 'Policy updated.', 'governdocs-document-governance' ),
            5  => $revision_id
                ? sprintf(
                    /* translators: %s: revision title */
                    esc_html__( 'Policy restored to revision from %s.', 'governdocs-document-governance' ),
                    wp_post_revision_title( $revision_id, false )
                )
                : false,
            6  => esc_html__( 'Policy published.', 'governdocs-document-governance' ) . $shortcode_hint,
            7  => esc_html__( 'Policy saved.', 'governdocs-document-governance' ) . $shortcode_hint,
            8  => esc_html__( 'Policy submitted.', 'governdocs-document-governance' ) . $shortcode_hint,
            9  => sprintf(
                /* translators: %s: scheduled date */
                esc_html__( 'Policy scheduled for: %s.', 'governdocs-document-governance' ),
                '<strong>' . esc_html( wp_date( 'M j, Y @ G:i', strtotime( $post->post_date ) ) ) . '</strong>'
            ) . $shortcode_hint,
            10 => esc_html__( 'Policy draft updated.', 'governdocs-document-governance' ) . $shortcode_hint,
        );
    ...
    public function register() : void {
        $args = $this->common_args(
            'governdocs_policy',
            __( 'Policy', 'governdocs-document-governance' ),
            __( 'Policies', 'governdocs-document-governance' ),
        ...
        register_post_type( 'governdocs_policy', $args );
    }

    public function register_taxonomies() : void {
        $post_types = array( 'governdocs_policy' );

        register_taxonomy(
            'governdocs_department',
            $post_types,
            array(
                'labels' => array(
                ...
                'rewrite'           => array( 'slug' => 'policy-department' ),
            )
        );
    }
}